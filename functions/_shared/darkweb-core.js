/**
 * BCyberAware – Dark Web Early Warning (Cloudflare Pages)
...
export async function handleDarkwebRequest(request) {
  ...
  try {
    ...
    if (!bust && CACHE.dw && (now - CACHE.dw.ts) < CACHE_MS)
      return jsonResponse({ ...CACHE.dw.data, from_cache: true });
