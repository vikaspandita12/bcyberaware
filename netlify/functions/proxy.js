/**
 * BCyberAware – Live Threat Feed (Netlify Edition)
 * Author  : Vikas Pandita | BCyberAware
 * Host    : Netlify (free — 125,000 function calls/month)
...
async function fetchCISAKEV() {
  ...
  return data.vulnerabilities
    ...
    .slice(0, 40) // increased from 20 → 40
    .map(v => ({
...
async function fetchNVDCritical() {
  try {
    ...
    const r     = await fetch(`https://services.nvd.nist.gov/rest/json/cves/2.0?pubStartDate=${start}&pubEndDate=${end}&cvssV3Severity=CRITICAL&resultsPerPage=25`,{headers:{"User-Agent":"BCyberAware/2.0"}});
    const data  = await r.json();
